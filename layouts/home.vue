...
<template>
  <div>
    <Header :isHome=true :menu="menuNavigation" :openModal="!isModalOpened" @openSearchModal="handleOpenModal"  />
    <div class="flex-fill">
      <slot />
    </div>
    <Modal
    ...
    >
      ...
      <template v-slot:content>
        <div class="relative p-3 border border-gray-200 rounded-lg w-full">
          <input type="search" class="rounded-md p-1 w-full focus:outline-none focus:focus-none" :placeholder="$t('search.placeholder')" v-model="searchQuery">
        </div>
