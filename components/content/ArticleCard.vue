...
<template>
  <div v-if="item" class="transition duration-300 ease-in-out hover:shadow-lg hover:scale-105 rounded-md h-64 overflow-hidden">
    <prismic-link :field="item" :aria-label="item.data.title">
      <div class="relative">
        <prismic-image v-if="imageVignette" :field="imageVignette" class="object-cover object-center h-full w-full" :width="imageVignette.dimensions?.width" :height="imageVignette.dimensions?.height" :aria-placeholder="[imageVignette.dimensions?.width, imageVignette.dimensions?.height]" />
